...
class VocabularyApp {
  ...
  renderExamplesForCurrentWord() {
    ...
    const links = [
      {
        name: "Tatoeba",
        url: `https://tatoeba.org/en/sentences/search?from=eng&to=pol&query=${q}`,
      },
      {
        name: "Linguee",
        url: `https://www.linguee.com/english-polish/search?source=english&query=${q}`,
      },
      {
        name: "Reverso",
        url: `https://context.reverso.net/translation/english-polish/${q}`,
      },
  ...
  saveData() {
    ...
    localStorage.setItem(
      "dynamicCategories",
      JSON.stringify(this.dynamicCategories)
    );
    localStorage.setItem(
      "categoryUsageStats",
  ...
  loadDefaultWords() {
    ...
    const hasOldWords = this.words.some(
      (word) => !word.aiGenerated && !word.hasImage
    );

    if (this.words.length === 0 || hasOldWords) {
      if (hasOldWords) {
        console.log(
          "Wykryto stare słowa bez obrazków - resetuję do AI słów..."
        );
  ...
  async generateInitialWords() {
    ...
    try {
      ...
      const initialWords = await this.getWordsFromAI(
        "dom",
  ...
  setupEventListeners() {
    ...
    document.querySelector(".flip-btn").addEventListener(
      "click",
      (e) => {
        const now = Date.now();
    ...
    document
      .getElementById("listening-input")
      .addEventListener("keypress", (e) => {
        this.userInteracted = true;
        if (e.key === "Enter") {
          this.checkListeningAnswer();
    ...
    document
      .getElementById("second-interval")
      .addEventListener("change", (e) => {
    ...
    document
      .getElementById("infinite-learning")
      .addEventListener("change", (e) => {
    ...
    // Auto-flip settings
    document
      .getElementById("auto-flip-enabled")
      .addEventListener("change", (e) => {
    ...
    document
      .getElementById("auto-flip-delay")
      .addEventListener("change", (e) => {
    ...
    const animSpeedValue = document.getElementById(
      "flashcard-anim-speed-value"
    );
    if (animSpeedInput && animSpeedValue) {
      const updateAnimLabel = (val) =>
        (animSpeedValue.textContent = `${val} ms`);
    ...
    document
      .getElementById("enable-ai-recommendations")
      .addEventListener("change", (e) => {
    ...
    document
      .getElementById("adaptive-difficulty")
      .addEventListener("change", (e) => {
    ...
    document
      .getElementById("test-ai-connection")
    ...
    window.addEventListener("resize", () => {
      if (
        document.getElementById("progress-view").classList.contains("active")
      ) {
  ...
  async prepareStudySet() {
    ...
    if (newWords.length > this.settings.dailyGoal) {
      this.currentStudySet = [
        ...reviewWords,
        ...newWords.slice(0, this.settings.dailyGoal),
      ];
    }
  }
    ...
    const speak = () => {
      ...
      if (!selectedVoice && utterance.lang) {
        selectedVoice = voices.find(
          (v) => v.lang && v.lang.toLowerCase() === utterance.lang.toLowerCase()
        );
      }
  ...
  playCompletionSound() {
    ...
    setTimeout(() => this.playTone(1046.5, 0.4, "sine"), 450); // C6
  }
  ...
  setupFlashcardGestures() {
    ...
    flashcard.addEventListener("touchend", (e) => {
      ...
      if (!isInteractive && isTap) {
        ...
        if (
          lastTapTime &&
          now - lastTapTime <= DOUBLE_TAP_MAX_DELAY &&
          Math.max(dxTap, dyTap) <= DOUBLE_TAP_MAX_DISTANCE
        ) {
  ...
  // Match Mode
  loadMatch() {
    const wordsToMatch = this.currentStudySet.slice(
  ...
  selectMatchWord(element) {
    ...
    document
      .querySelectorAll(`.match-word[data-type="${type}"].selected`)
      .forEach((el) => {
  ...
  checkSingleMatch() {
    ...
    const correctPair = this.currentStudySet.find(
      (w) => w.polish === polishWord && w.english === englishWord
    );

    const polishEl = document.querySelector(
      `.match-word[data-word="${polishWord}"]`
    );
    const englishEl = document.querySelector(
      `.match-word[data-word="${englishWord}"]`
    );
    ...
    if (correctPair) {
      ...
      const polishWordObj = this.currentStudySet.find(
        (w) => w.polish === polishWord
      );
      if (polishWordObj) {
  ...
  checkIfAllMatched() {
    // Sprawdź czy wszystkie słowa zostały dopasowane
    const remainingPolish = document.querySelectorAll(
      "#polish-words .match-word"
    ).length;
    const remainingEnglish = document.querySelectorAll(
      "#english-words .match-word"
    ).length;
    const totalWords = this.currentStudySet.slice(
      this.currentWordIndex,
      this.currentWordIndex + 5
    ).length;
...
const proxify = (url) => {
    ...
    try {
      ...
      if (provider === "imagen-4") {
        if (this.settings.enableImagen && this.supportsImagenServer()) {
          const url = await this.generateImageWithImagen4(
            englishWord,
            polishWord
          );
      ...
      if (provider === "gemini-flash-preview") {
        if (this.settings.aiApiKey || this.supportsImagenServer()) {
          const url = await this.generateImageWithGeminiFlashPreview(
            englishWord,
            polishWord
          );
          if (url) return applyImage(url);
      ...
      const aiImageUrl = await this.generateFreeAIImage(
        englishWord,
        polishWord
      );
      if (aiImageUrl) {
        this.imageUrlCache[cacheKey] = aiImageUrl;
        return applyImage(aiImageUrl);
      }
      return this.loadFallbackImage(container, englishWord, polishWord);
    } catch (error) {
  ...
  prefetchNextImages(count = 3) {
    try {
      if (
        !Array.isArray(this.currentStudySet) ||
        this.currentStudySet.length === 0
      )
        return;
      const start = Math.max(0, (this.currentWordIndex || 0) + 1);
      const end = Math.min(
        this.currentStudySet.length,
        start + Math.max(0, count | 0)
      );
  ...
  async prefetchImageForWord(word) {
    try {
      ...
      if (
        !english ||
        this.imageUrlCache[key] ||
        this._prefetchInFlight.has(key)
      )
        return;
      ...
      try {
        this._prefetchInFlight.delete(
          `${String(word.english || "").toLowerCase()}|${String(
            word.polish || ""
          ).toLowerCase()}`
        );
      } catch (_) {}
      ...
      if (canUseAI) {
        ...
        try {
          ...
          if (provider === "openai") resp = await this.callOpenAI(prompt);
            ...
            resp = await this.callAnthropic(prompt);
          else if (provider === "gemini")
            resp = await this.callGemini(prompt, {
              responseMimeType: "text/plain",
            });
          definition = String(resp || "")
            .split("\n")[0]
            .trim();
...
// Metoda klasy VocabularyApp (bez słowa kluczowego 'function')
  ...
  async generateImageWithImagen4(englishWord, polishWord) {
    try {
      ...
      const prompt = `A simple, clear illustration of ${englishWord} (${sense}), clean white background, flat vector, clipart style. Absolutely no text, no words, no letters, no captions, no signatures, no watermark, no fonts, no typography. The image must be label-free and pictorial only.`;
  ...
  supportsImagenServer() {
    try {
      return !!(
        this.settings &&
        this.settings.proxyBaseUrl &&
        /^https?:\/\//.test(this.settings.proxyBaseUrl)
      );
    } catch (_) {
      return false;
    }
  ...
  async generateImageWithGeminiFlashPreview(englishWord, polishWord) {
    try {
      const sense = await this.getImageSenseText(englishWord, polishWord);
      const prompt = `A simple, clear illustration of ${englishWord} (${sense}), clean white background, flat vector, clipart style. Important: Do not include any text, words, letters, captions, watermarks, or typography. The image must be purely pictorial and label-free.`;
      ...
      if (this.supportsImagenServer()) {
        ...
        if (!response.ok) {
          const text = await response.text().catch(() => "");
          throw new Error(
            `Gemini Flash Preview proxy error: ${response.status} ${text}`
          );
        }
        const data = await response.json();
        if (data?.base64) {
      ...
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(
        model
      )}:generateContent?key=${encodeURIComponent(this.settings.aiApiKey)}`;
      ...
      if (!response.ok) {
        const text = await response.text().catch(() => "");
        throw new Error(
          `Gemini Flash Preview error: ${response.status} ${text}`
        );
      }
      const data = await response.json();
      const parts = data?.candidates?.[0]?.content?.parts || [];
      const imgPart = parts.find(
        (p) => p.inlineData && p.inlineData.mimeType && p.inlineData.data
      );
      ...
      const linkPart = parts.find(
        (p) => typeof p.text === "string" && p.text.startsWith("http")
      );
  ...
  async recordAnswer(isCorrect, word = null) {
    ...
    const interval = await this.getAIRecommendedInterval(
      currentWord,
      isCorrect
    );
    ...
    if (isCorrect) {
      ...
      currentWord.nextReview = new Date(
    ...
    const wordCategory =
      currentWord.category || this.guessWordCategory(currentWord.polish);
  ...
  updateLearningPatterns(word, isCorrect) {
    ...
    if (category !== "inne") {
      const categoryIndex = this.learningPatterns.preferredCategories.findIndex(
        (c) => c.name === category
      );
      if (categoryIndex >= 0) {
        this.learningPatterns.preferredCategories[categoryIndex].successRate =
          (this.learningPatterns.preferredCategories[categoryIndex]
            .successRate +
            (isCorrect ? 1 : 0)) /
          2;
      } else {
  ...
  updateProgress() {
    ...
    const progress =
      (this.currentWordIndex / this.currentStudySet.length) * 100;
  ...
  updateStats() {
    ...
    document.getElementById("learned-words").textContent =
      this.stats.learnedWords;
    document.getElementById("due-words").textContent = this.stats.dueWords;
    document.getElementById(
      "success-rate"
    ).textContent = `${this.stats.successRate}%`;

    // Update AI info
    document.getElementById("current-level").textContent =
      this.settings.languageLevel;
    document.getElementById("infinite-status").textContent = this.settings
      .infiniteLearning
      ? "🚀"
      : "⏸️";
  ...
  calculateStats() {
    ...
    this.stats.learnedWords = this.words.filter(
      (w) => w.status === "learned"
    ).length;
  ...
  updateCategoryDisplay() {
    ...
    if (this.dynamicCategories.length > 0) {
      console.log(
        `📚 Kategorie: ${totalCategories} łącznie (${dynamicCategories} dynamicznych AI)`
      );
      console.log(
        `🎯 Ostatnie AI kategorie:`,
        this.dynamicCategories.slice(-3)
      );
    }
  }
  ...
  renderProgressChart() {
    ...
    const data = [
      {
        label: "Nowe",
        value: this.words.filter((w) => w.status === "new").length,
        color: "#1976d2",
      },
      {
        label: "Uczę się",
        value: this.words.filter((w) => w.status === "learning").length,
        color: "#f57c00",
      },
      {
        label: "Nauczone",
        value: this.words.filter((w) => w.status === "learned").length,
        color: "#388e3c",
      },
    ];
  ...
  async generateDynamicCategory() {
    if (!this.settings.aiApiKey && this.settings.aiProvider !== "free") {
      console.warn(
        "Brak klucza API - nie można generować dynamicznych kategorii"
      );
      return null;
    }
...
- Słabe obszary użytkownika: ${
        weakAreas.length > 0 ? weakAreas.join(", ") : "brak danych"
      }
- Poziom: ${userLevel}
...
Odpowiedź (tylko nazwa kategorii):`;
      ...
      switch (this.settings.aiProvider) {
        case "free":
          response = await this.generateCategoryFromTemplate(
            userLevel,
            weakAreas
          );
          break;
  ...
  generateCategoryFromTemplate(level, weakAreas) {
    ...
    const availableCategories =
      categoryTemplates[level] || categoryTemplates["B1"];
    const unusedCategories = availableCategories.filter(
      (cat) => !this.wordCategories.includes(cat)
    );
  ...
  async shouldGenerateNewCategory() {
    ...
    const daysSinceLastCategory = lastDynamicCategory
      ? (Date.now() - new Date(lastDynamicCategory).getTime()) /
        (1000 * 60 * 60 * 24)
      : 999;
  ...
  getLastDynamicCategoryDate() {
    ...
    const lastCategory =
      this.dynamicCategories[this.dynamicCategories.length - 1];
  ...
  selectBestExistingCategory() {
    ...
    const categoryScores = this.wordCategories.map((category) => {
      ...
      const wordsInCategory = this.words.filter(
    ...
    console.log(
      ...
      } (wynik: ${selectedCategory.score.toFixed(1)})`
    );
  ...
  async generateNewWords(count = 5) {
    ...
    try {
      let newWords = await this.getWordsFromAI(
        category,
        this.settings.languageLevel,
        count
      );
      ...
      if (
        this.settings.enableAISelfCheck &&
      ...
      const existingKeys = new Set(
        this.words.map(
          (w) => `${w.polish.toLowerCase()}-${w.english.toLowerCase()}`
        )
      );
      ...
      if (uniqueWords.length < count) {
        const additionalCategories = this.wordCategories.filter(
          (c) => c !== category
        );
  ...
  async getFreeAIWords(category, level, count) {
    console.log(
      `Generowanie darmowych słów AI: ${count} słów z kategorii "${category}" poziom ${level}`
    );
    ...
    try {
      ...
      for (let i = 0; i < count; i++) {
        ...
        const contextPrompt = `${difficulty} ${category} vocabulary word ${
          i + 1
        }`;
        ...
        try {
          ...
          const englishWord = await this.generateRelatedWord(
            category,
            level,
            i
          );
  ...
  async generateRelatedWord(category, level, index) {
    ...
    const categoryWords =
      thematicWords[category]?.[level] || thematicWords["dom"]["A1"];
  ...
  getWordsByLevel(
    ...
    console.log(
      `Fallback: generowanie ${count} słów dla kategorii ${category} na poziomie ${level}`
    );
    ...
    const availableWords = fallbackWords.filter(
      (word) => !existingWords.includes(word.english.toLowerCase())
    );
    ...
    return availableWords
      .slice(0, Math.min(count, availableWords.length))
      .map((word) => ({
        ...word,
        status: "new",
        attempts: 0,
  ...
  buildAIPrompt(category, level, count) {
    const existingWords = this.words
      .map((w) => w.english)
      .slice(-20)
      .join(", ");
        ...
        messages: [
          {
            role: "system",
            content:
              "Jesteś ekspertem w nauczaniu języka angielskiego. Odpowiadaj tylko w formacie JSON.",
          },
          {
            role: "user",
            content: prompt,
  ...
  async selfCheckAndFixTranslations(words) {
    try {
      ...
      const examples = words
        .slice(0, 20)
        .map((w) => ({ polish: w.polish, english: w.english }));
...
- If multiple senses exist, pick the sense that corresponds to the Polish term (like a dictionary headword). Avoid paraphrases unless necessary.`;
      ...
      switch (this.settings.aiProvider) {
        case "openai":
          respText = await this.callOpenAI(prompt);
          break;
        case "anthropic":
          respText = await this.callAnthropic(prompt);
          break;
        case "gemini":
          respText = await this.callGemini(prompt);
          break;
        default:
          return words;
      }
...
Polish translation:`;
  ...
  async parseAIResponse(response, category = "inne") {
    ...
    try {
      ...
      // Walidacja elementów
      const validWords = parsed.filter(
        (item) =>
          typeof item === "object" &&
          item !== null &&
          typeof item.polish === "string" &&
          item.polish.trim() !== "" &&
      ...
      if (validWords.length !== parsed.length) {
        console.warn(
          "Niektóre obiekty od AI miały nieprawidłowy format i zostały odrzucone."
        );
      }
      ...
      try {
        ...
        if (results.length > 0) {
          console.warn(
            "JSON od AI był niepoprawny – odzyskano pary poprzez analizę tekstu."
          );
  ...
  analyzeUserProgress() {
    ...
    const learnedWords = this.words.filter(
      (w) => w.status === "learned"
    ).length;
  ...
  analyzeCategoryPerformance() {
    ...
    this.wordCategories.forEach((category) => {
      const categoryWords = this.words.filter(
        (w) =>
          w.category === category ||
          this.guessWordCategory(w.polish) === category
      );

      if (categoryWords.length > 0) {
        const totalAttempts = categoryWords.reduce(
          (sum, w) => sum + w.attempts,
          0
        );
        const totalCorrect = categoryWords.reduce(
          (sum, w) => sum + w.correct,
          0
        );
  ...
  guessWordCategory(polishWord) {
    ...
    for (const [category, keywords] of Object.entries(categoryKeywords)) {
      if (
        keywords.some((keyword) => polishWord.toLowerCase().includes(keyword))
      ) {
        return category;
      }
    }
...
Zwróć tylko liczbę dni (1-30) jako interwał do następnej powtórki:`;
  ...
  // Settings
  loadSettings() {
    document.getElementById("first-interval").value =
      this.settings.firstInterval;
    document.getElementById("second-interval").value =
      this.settings.secondInterval;
    document.getElementById("daily-goal").value = this.settings.dailyGoal;
    document.getElementById("language-level").value =
      this.settings.languageLevel;
    document.getElementById("infinite-learning").checked =
      this.settings.infiniteLearning;
    ...
    document.getElementById("auto-flip-delay").value =
    ...
    if (imageProviderSelect) {
      ...
      const imagenOpt = imageProviderSelect.querySelector(
        'option[value="imagen-4"]'
      );
      const flashOpt = imageProviderSelect.querySelector(
        'option[value="gemini-flash-preview"]'
      );
      if (imagenOpt) {
        imagenOpt.disabled = true;
        imagenOpt.textContent = "Imagen 4 (wyłączone)";
      }
      if (flashOpt) {
        flashOpt.disabled = true;
        flashOpt.textContent = "Gemini 2.5 Flash Preview (wyłączone)";
      }
    ...
    document.getElementById("enable-ai-recommendations").checked =
      this.settings.enableAIRecommendations;
    document.getElementById("adaptive-difficulty").checked =
  ...
  updateAIModelOptions() {
    ...
    if (currentProvider === "free") {
      desired = allowedValues.includes("mymemory")
        ? "mymemory"
        : allowedValues[0];
    } else if (currentProvider === "gemini") {
      if (
        !desired ||
        !String(desired).startsWith("gemini-") ||
        !allowedValues.includes(desired)
      ) {
        desired = allowedValues.includes("gemini-2.5-flash")
          ? "gemini-2.5-flash"
          : allowedValues[0];
      }
    } else if (currentProvider === "openai") {
      if (!desired || !allowedValues.includes(desired)) {
        desired = allowedValues.includes("gpt-3.5-turbo")
          ? "gpt-3.5-turbo"
          : allowedValues[0];
      }
    } else if (currentProvider === "anthropic") {
      if (!desired || !allowedValues.includes(desired)) {
        desired = allowedValues.includes("claude-3-haiku-20240307")
  ...
  exportWords() {
    const csv =
      "Polski,Angielski,Status,Próby,Poprawne\n" +
      this.words
        .map(
          (w) =>
            `"${w.polish}","${w.english}","${w.status}",${w.attempts},${w.correct}`
        )
        .join("\n");
  ...
  resetProgress() {
    if (
      confirm(
        "Czy na pewno chcesz zresetować postępy? Wszystkie słowa zostaną oznaczone jako nowe."
      )
    ) {
      this.words.forEach((word) => {
        word.status = "new";
